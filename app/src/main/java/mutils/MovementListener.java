...
public class MovementListener implements SensorEventListener {
    ...
    public interface ILegMovementListener {
         ...
         */
        void onLegActivity(int activity, float value);
    }
    ...
    public MovementListener(SensorManager sensorManager){
     ...
     */
    private void notifyListeners(int activity, float value){
        if (activity == 0) return;
        for (ILegMovementListener listener : mListeners){
            listener.onLegActivity(activity, value);
